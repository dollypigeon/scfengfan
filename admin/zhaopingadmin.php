...
$sql = "SELECT id,jobname  FROM pwn_job";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {
        echo "<form action='zhaopingdetail.php' > <label>id: " . $row["id"]. " - Name: " . $row["jobname"]. "</label><br> <button type='submit' name='id' value='" . $row["id"]. "'>Change</button></form>";
    }
} else {
    echo "0 results";
}
